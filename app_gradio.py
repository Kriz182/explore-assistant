...
vertexai.init(project="GCP-PROJECT", location="LOCATION")
parameters = {
    "temperature": 0.2,
...
# Update with the relevant model
...
def search(query):
    ...
    url = f"{looker_base_url}{response}&toggle=dat,pik,vis&allow_login_screen=true"
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        ...
        #    button = gr.Button(value="Add to Examples")
        #    button_2 = gr.Button(value="Another Action")
    with gr.Row():
        search_results = gr.HTML()
        search_input.submit(search, inputs=search_input, outputs=search_results)
        ##search_input.click
    #with gr.Row():
    #   test = gr.TextArea(label="Generated Results", lines=5, placeholder = "Results of the search will be outputed here")
    #   gr.TextArea()
